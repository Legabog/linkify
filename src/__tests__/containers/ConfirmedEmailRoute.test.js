...
describe("ConfirmedEmailRoute", () => {
  it("header should have <h1> with i18n locales", async () => {
    ...
    await waitFor(() =>
      expect(
        getByText("unauthorized.confirmed-email-route.header.h1")
      ).toBeDefined()
    );
  });
  it("body should have <span> with i18n locales", async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(
          "unauthorized.confirmed-email-route.body.h2-1 unauthorized.confirmed-email-route.body.h2-2"
        )
      ).toBeDefined()
    );
  });
